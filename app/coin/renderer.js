 ...
 * Each cell = colored square. Activity shown via brightness.
 */
  ...
  render(getCell) {
    ...
    for (let cy = 0; cy < boardSize; cy++) {
      for (let cx = 0; cx < boardSize; cx++) {
        ...
        // Boost brightness slightly for active cells
        const boost = 1.0 + cell.activity * 0.3;
        const fr = Math.min(255, (r * boost) | 0);
        const fg = Math.min(255, (g * boost) | 0);
        const fb = Math.min(255, (b * boost) | 0);

        // Fill cellPx x cellPx block
        const px0 = cx * cellPx;
        const py0 = cy * cellPx;
