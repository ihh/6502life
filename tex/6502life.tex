...
\item A BRK software interrupt is handled almost identically, with the following differences:
  \begin{itemize}
    ...
    \item After saving registers Y,X,A,S to memory, the operand byte $A$ following the BRK opcode is examined.
      Let $N^2 = 49$ (the number of cells in the 7x7 neighborhood).
      The operand encodes a source cell $s = \lfloor A / N^2 \rfloor$ and a destination cell $d = A \bmod N^2$,
      ...
      If $1 \leq A < 245$ and $s \neq d$, cells $s$ and $d$ are swapped (1024-byte blocks each).
      Otherwise ($A = 0$, $s = d$, or $A \geq 245$) nothing happens.
