...
\item A BRK software interrupt is handled almost identically, with the following differences:
  \begin{itemize}
    ...
    \item After saving registers Y,X,A,S to memory, the operand byte $b$ (the byte in memory immediately following the BRK opcode) is examined.
      ...
      The noisy copy operation (operands 245--252) copies the 1024-byte origin cell
      to a destination cell. Each bit in the destination is determined by a hierarchical
      coin-flip model governed by six board-level probability parameters:
      $p_{\mathrm{CellMask}}$, $p_{\mathrm{CellNoise}}$,
      $p_{\mathrm{ByteMask}}$, $p_{\mathrm{ByteNoise}}$,
      $p_{\mathrm{BitMask}}$, $p_{\mathrm{BitNoise}}$.

      At each level of the hierarchy (cell, byte, bit), two independent coins are
      flipped using the board PRNG:
      \begin{enumerate}
      \item \textbf{Cell level.}
        Flip $p_{\mathrm{CellMask}}$: if heads, the entire destination cell is untouched (copy aborted).
        Otherwise flip $p_{\mathrm{CellNoise}}$: if heads, the entire cell is filled with pseudorandom bytes.
      \item \textbf{Byte level} (for each of the 1024 bytes).
        Flip $p_{\mathrm{ByteMask}}$: if heads, the byte is untouched.
        Otherwise flip $p_{\mathrm{ByteNoise}}$: if heads, the byte is replaced with a pseudorandom byte.
      \item \textbf{Bit level} (for each of the 8 bits in the byte).
        Flip $p_{\mathrm{BitMask}}$: if heads, the destination bit is untouched.
        Otherwise flip $p_{\mathrm{BitNoise}}$: if heads, the bit is sampled uniformly
        from $\{0,1\}$. If tails, the bit is set to the corresponding source bit (faithful copy).
      \end{enumerate}

      Coarser levels override finer levels: a cell-level decision preempts all byte-
      and bit-level processing. Default values are $p_{\mathrm{BitNoise}} = 0.001$
      with all other probabilities zero, yielding a near-faithful copy with
      approximately 8 mutated bits per cell.
    ...
    \item After any swap is performed, control returns to the scheduler, which will randomly pass control to another cell.
