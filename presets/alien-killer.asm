...
; If it doesn't match, JAM the neighbor's entry point, redirect its PC,
; then copy self over it and reset its registers.
;
...
; --- Phase 1: JAM the neighbor (prevents it executing if we're interrupted) ---
  ...
  LDA #$00
  STA $04F9           ; set neighbor's saved PC high = 0
  STA $04FA           ; set neighbor's saved PC low = 0
...
; --- Phase 3: Reset neighbor's stack pointer ---
  LDA #$FF
  STA $04FF
