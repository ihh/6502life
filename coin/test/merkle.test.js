...
import { generateChallenges, respondToChallenge, verifyChallenge } from '../challenge.js';
import { BoardContract } from '../contract.js';
...
describe('Challenge protocol', () => {
  const keyA = sha256(new TextEncoder().encode('keyA'));
  const keyB = sha256(new TextEncoder().encode('keyB'));

  it('generates deterministic challenges from same keys', () => {
    const c1 = generateChallenges(keyA, keyB, 5, 1000);
    const c2 = generateChallenges(keyA, keyB, 5, 1000);
    expect(c1).toEqual(c2);
  });

  it('generates same challenges regardless of key order', () => {
    const c1 = generateChallenges(keyA, keyB, 5, 1000);
    const c2 = generateChallenges(keyB, keyA, 5, 1000);
    expect(c1).toEqual(c2);
  });

  it('generates the requested number of unique challenges', () => {
    const challenges = generateChallenges(keyA, keyB, 10, 10000);
    expect(challenges.length).toBe(10);
    expect(new Set(challenges).size).toBe(10);
  });

  it('all challenges are within [0, maxTick)', () => {
    const challenges = generateChallenges(keyA, keyB, 20, 500);
  ...
  it('challenges are returned sorted', () => {
    const challenges = generateChallenges(keyA, keyB, 10, 10000);
    for (let i = 1; i < challenges.length; i++) {
      expect(challenges[i]).toBeGreaterThanOrEqual(challenges[i - 1]);
    }
  });

  it('respondToChallenge returns valid proofs', () => {
    const tree = new BoardMerkleTree();
    ...
    const root = tree.root();
    // Use a subset of ticks that exist in the tree
    const challengeTicks = [0, 30, 70, 100];
    const response = respondToChallenge(tree, challengeTicks);

    expect(response.length).toBe(4);
    expect(verifyChallenge(root, response)).toBe(true);
  });

  it('verifyChallenge rejects tampered response', () => {
    const tree = new BoardMerkleTree();
    for (let i = 0; i <= 10; i++) {
      tree.append(stateHashAt(i * 10), i * 10);
    }
    ...
    const response = respondToChallenge(tree, [0, 50, 100]);

    // Tamper with one proof's state hash
    response[1].stateHash = 'ab'.repeat(32);
    expect(verifyChallenge(root, response)).toBe(false);
  });

  it('handles edge cases for empty/zero inputs', () => {
    expect(generateChallenges(keyA, keyB, 0, 1000)).toEqual([]);
    expect(generateChallenges(keyA, keyB, 5, 0)).toEqual([]);
  });
});
...
describe('BoardContract', () => {
  ...
  it('creates a contract with correct fields', () => {
    const { publicKey } = makeKeyPair();
    const rawPub = getRawPublicKey(publicKey);

    const contract = new BoardContract({
      boardParams: { width: 256, height: 256, seed: 42 },
      initialStateHash: 'aa'.repeat(32),
      maxMovesPerDay: 100,
      maxMoveSize: 1024,
      ownerPublicKey: rawPub
    });

    expect(contract.boardParams.width).toBe(256);
    expect(contract.initialStateHash).toBe('aa'.repeat(32));
    expect(contract.signature).toBeNull();
  });

  it('signs and verifies a contract', () => {
    const { publicKey, privateKey } = makeKeyPair();
    const rawPub = getRawPublicKey(publicKey);

    const contract = new BoardContract({
      boardParams: { width: 256, height: 256, seed: 42 },
      initialStateHash: 'bb'.repeat(32),
      maxMovesPerDay: 100,
      maxMoveSize: 1024,
      ownerPublicKey: rawPub
    });
    ...
    expect(contract.verify()).toBe(true);
  });

  it('verification fails with wrong public key', () => {
    const { privateKey } = makeKeyPair();
    const { publicKey: otherPublic } = makeKeyPair();
    const wrongPub = getRawPublicKey(otherPublic);

    const contract = new BoardContract({
      boardParams: { width: 32, height: 32, seed: 1 },
      initialStateHash: 'cc'.repeat(32),
      maxMovesPerDay: 50,
      maxMoveSize: 512,
      ownerPublicKey: wrongPub
    });

    // Sign with a different key than the one embedded
    contract.sign(privateKey);
    expect(contract.verify()).toBe(false);
  });

  it('unsigned contract fails verification', () => {
    const { publicKey } = makeKeyPair();
    const rawPub = getRawPublicKey(publicKey);

    const contract = new BoardContract({
      boardParams: { width: 16, height: 16, seed: 0 },
      initialStateHash: 'dd'.repeat(32),
      maxMovesPerDay: 10,
      maxMoveSize: 256,
      ownerPublicKey: rawPub
    });
  ...
  it('serialize/deserialize round-trip preserves signature', () => {
    const { publicKey, privateKey } = makeKeyPair();
    const rawPub = getRawPublicKey(publicKey);

    const contract = new BoardContract({
      boardParams: { width: 64, height: 64, seed: 7 },
      initialStateHash: 'ee'.repeat(32),
      maxMovesPerDay: 200,
      maxMoveSize: 2048,
      ownerPublicKey: rawPub
    });
    ...
    contract.sign(privateKey);
    const serialized = contract.serialize();
    const restored = BoardContract.deserialize(serialized);

    expect(restored.verify()).toBe(true);
    expect(restored.boardParams).toEqual(contract.boardParams);
    expect(restored.initialStateHash).toBe(contract.initialStateHash);
    expect(restored.maxMovesPerDay).toBe(contract.maxMovesPerDay);
  });

  it('tampered serialized contract fails verification', () => {
    const { publicKey, privateKey } = makeKeyPair();
    const rawPub = getRawPublicKey(publicKey);
    ...
    // Tamper with the initial state hash
    serialized.initialStateHash = 'ff'.repeat(32);
    const restored = BoardContract.deserialize(serialized);
    expect(restored.verify()).toBe(false);
  });
});
