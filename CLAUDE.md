...
- **Board hyperparameters** (`boardParams`): scalar params `pBitNoise` (default 1/2048), `pBitNoiseZero` (default 0.5, P(resampled bit=0)), `decayRate` (default 0, per-bit flip probability per cycle per cell — cosmic rays — converted to per-quantum via ×4096 mean cycles, Poisson-sampled using board PRNG), `hasCompass` (default false), `schedulerMode` (default `'random'`). The `brkOps` registry (see below) controls which BRK operands are enabled. Each BRK op has a cycle cost; if fewer cycles remain before the next timer IRQ than the op requires, the BRK silently fails.
- **Scheduler modes** (`schedulerMode`): `'random'` picks a random cell each quantum (default). `'checkerboard'` cycles through all cells in two deterministic passes per round: first all cells where `(i+j)%2==0` (row-major), then all cells where `(i+j)%2==1`. Within each pass, no two cells are cardinal neighbors, enabling future GPU parallelism. Orientation and timer duration are still random per cell.
