...
- **Board hyperparameters** (`boardParams`): scalar params `pBitNoise` (default 1/2048), `pBitNoiseZero` (default 0.5, P(resampled bit=0)), `decayRate` (default 0, per-bit flip probability per quantum — cosmic rays — applied to entire board storage via Poisson sampling using board PRNG), `hasCompass` (default false), `schedulerMode` (default `'random'`). The `brkOps` registry (see below) controls which BRK operands are enabled. Each BRK op has a cycle cost; if fewer cycles remain before the next timer IRQ than the op requires, the BRK silently fails.
